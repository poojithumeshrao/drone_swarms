...
from mavros_msgs.msg import State,Waypoint,WaypointList,CommandCode,control 
...
waypoints_of_all = [[] for i in range(num_uav)]
...
def dynamic_update(data):
    wp_list = waypoints_of_all[data.uav_no].waypoints
    pdb.set_trace()
    n = data.uav_no
    
    uav = "uav"+str(data.uav_no + 1)
    p = 0
    for wp in wp_list:
        if wp.is_current == True:
            wp.is_current = False
            wp_list.insert(p,data.waypoint)
            break
...
rospy.Subscriber("contoller",control,dynamic_update)
